...
### 2. Configurar los Archivos de Entorno y Secretos Locales
...
spring:
  profiles:
    active: dev,local
```
- `dev`: Se conecta a la base de datos de desarrollo y habilita logs detallados de Hibernate.
...
#### Empaquetar en archivo ejecutable JAR:
```bash
mvn clean package -DskipTests
java -jar target/gestor-salon-eventos-0.1.0-SNAPSHOT.jar
```

#### Desde tu IDE (IntelliJ IDEA, VS Code, Eclipse):
